...
export function buildApp(settings: Settings, db: Db, fetcher: typeof fetch = fetch): Hono {
  ...
  app.post("/api/v1/owner/service-keys", async (context) => { const current=await owner(context); if(current instanceof Response)return current; const body=await context.req.json().catch(()=>({})); try{return context.json(await createKey(db,settings,current,{name:String(body.name??"Service key"),backend:"codex_subscription"}),201);}catch(error){return bad(context,400,"create_failed",error instanceof Error?error.message:"create failed");} });
  ...
  app.post("/api/v1/owner/codex/login", async (context) => { const current=await owner(context); if(current instanceof Response)return current; try{return context.json(await proxy.appServer.loginStart());}catch(error){return bad(context,502,"codex_login_unavailable",error instanceof Error?error.message:"Unable to start Codex login.");} });
